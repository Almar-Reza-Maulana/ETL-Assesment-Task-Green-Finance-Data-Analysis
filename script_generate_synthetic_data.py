# Cara Kerja Proses
# 1. Import Library dan Inisialisasi Fungsi Bantuan
# Mengimpor pandas, numpy, random, dan string.
# Definisi fungsi rand_float untuk membuat angka float acak dalam rentang tertentu dengan presisi decimal.
# Fungsi generate_project_id untuk membuat ID proyek baru dalam format PLTS-REGION-XXX berdasarkan region yang diambil dari ID proyek yang sudah ada agar tetap konsisten.

# 2. Baca Data Asli
# Membaca tiga dataset Excel: ekonomi, lingkungan, dan keuangan.
# Mendapatkan daftar Project_ID yang umum atau ada di ketiga dataset supaya konsistensi terjaga.

# 3. Generate Project_ID Baru
# Dengan generate_project_id, menghasilkan ID baru yang unik sehingga total jumlah baris proyek mencapai 100.
# ID baru dihasilkan dengan mencoba menyesuaikan region yang sudah ada agar logis.

# 4. Definisi Kategori dan Rentang Nilai Per Dataset
# Untuk setiap dataset (Economic, Environmental, Financial), didefinisikan kategori label dan rentang nilai variabel-variabel numeric terkait.
# Contoh: kategori ekonomi seperti "High: 💵💵💵💵" dengan nilai GDP_Growth dalam (5.35, 5.65), dst.

# 5. Generate Baris Data Baru Per Dataset
# Terdapat fungsi create_economic_row, create_environmental_row, dan create_financial_row yang:
# Memilih secara acak satu kategori dari kategori yang didefinisikan.
# Menghasilkan nilai numeric acak dalam rentang kategori tersebut.
# Mengembalikan dictionary baris data lengkap yang siap dipakai.

# 6. Membuat List Baris Baru untuk ID Proyek Baru
# Menggunakan fungsi di atas untuk membuat data baru sebanyak ID proyek baru yang dibutuhkan.
# Menghasilkan data baru yang konsisten dengan kategori dan region.

# 7. Gabungkan Data Lama dan Data Baru
# Menggabungkan data asli dan data baru menggunakan pd.concat.
# Membentuk dataset baru yang sudah diperluas.

# 8. Simpan ke File Excel Baru
# Menyimpan dataset hasil gabungan tersebut menjadi file Excel baru dengan nama output_*.xlsx.

# Tujuan Script
# Memperluas dataset proyek energi hijau menjadi minimal 100 proyek per dataset.
# Menjaga konsistensi Project_ID antar ketiga dataset, penting untuk analisis lintas dataset.
# Menghasilkan data sintetis realistis sesuai aturan bisnis yang diberikan (kategori dan rentang nilai).
# Mempermudah analisis yang memerlukan contoh data lebih banyak atau untuk simulasi/modeling.
# Menyediakan dataset lengkap yang siap digunakan untuk analisis statistik, machine learning, reporting, dan lain-lain.
